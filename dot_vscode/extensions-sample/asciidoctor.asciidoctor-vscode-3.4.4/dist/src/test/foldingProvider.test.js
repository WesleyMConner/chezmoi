...
foo2
    ...
    suite('getOpenBlockFoldingRanges', () => {
        test('Should fold open block', async () => {
            const folds = await getFoldsForDocument(`this is a paragraph
...
this is a paragraph`);
        ...
        test('Should not fold on title if it is not the block title ', async () => {
            const folds = await getFoldsForDocument(`.Title
This is a paragraph.
--
Open
--`);
...
this is a paragraph`);
        ...
        test('Should not fold single line comment if not contiguous ', async () => {
            const folds = await getFoldsForDocument(`// A single-line comment.
...
async function getFoldsForDocument(contents) {
    ...
    const provider = new foldingProvider_1.default(new asciidocLoader_1.AsciidocLoader(new asciidoctorConfig_1.AsciidoctorConfig(), new asciidoctorExtensions_1.AsciidoctorExtensions(security_1.AsciidoctorExtensionsSecurityPolicyArbiter.activate(helper_1.extensionContext)), new asciidoctorDiagnostic_1.AsciidoctorDiagnostic('text'), helper_1.extensionContext));

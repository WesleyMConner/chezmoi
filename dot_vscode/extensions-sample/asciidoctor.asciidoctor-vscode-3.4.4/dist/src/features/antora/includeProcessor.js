...
exports.IncludeProcessor = void 0;
class $Antora {
}
const DBL_COLON = '::';
const DBL_SQUARE = '[]';
...
exports.IncludeProcessor = (() => {
    const $callback = Symbol('callback');
    const classDef = global.Opal.klass(globalThis.Opal.Antora || global.Opal.module(null, 'Antora', $Antora), global.Opal.Asciidoctor.Extensions.IncludeProcessor, 'IncludeProcessor');
...
function filterLinesByTags(reader, target, file, tags, sourceCursor) {
            ...
            if (m[1]) {
                if (thisTag === activeTag) {
                    ...
                    if (~idx) {
                        ...
                        log('warn', `mismatched end tag (expected '${activeTag}' but found '${thisTag}') ` +
                            `at line ${lineNum} of include file: ${file.file})`, reader, sourceCursor, createIncludeCursor(reader, file, target, lineNum));
                    }
                    else {
                        log('warn', `unexpected end tag '${thisTag}' at line ${lineNum} of include file: ${file.file}`, reader, sourceCursor, createIncludeCursor(reader, file, target, lineNum));
                    }
                }
            }
            else if (tags.has(thisTag)) {
    ...
    if (tagStack.length) {
        tagStack.forEach(([tagName, _, tagLineNum]) => log('warn', `detected unclosed tag '${tagName}' starting at line ${tagLineNum} of include file: ${file.file}`, reader, sourceCursor, createIncludeCursor(reader, file, target, tagLineNum)));
    }
    if (foundTags.length)
        foundTags.forEach((name) => tags.delete(name));
    if (tags.size) {
        log('warn', `tag${tags.size > 1 ? 's' : ''} '${[...tags.keys()].join(', ')}' not found in include file: ${file.file}`, reader, sourceCursor, createIncludeCursor(reader, file, target, 0));
    }
    return [lines, startLineNum || 1];
}
function createIncludeCursor(reader, { file, src }, path, lineno) {
    ...
    // eslint-disable-next-line no-new-wrappers
    Object.assign(new String(file), { src, parent: { file: reader.file, lineno: reader.lineno - 1 } }), path, lineno);
}

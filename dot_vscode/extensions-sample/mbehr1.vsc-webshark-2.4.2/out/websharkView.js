...
exports.WebsharkView = exports.WebsharkViewReadonlyEditorProvider = exports.WebsharkViewSerializer = exports.SharkdProcess = void 0;
...
class SharkdProcess {
    constructor(sharkdPath, wiresharkProfile) {
        this.sharkdPath = sharkdPath;
        this.wiresharkProfile = wiresharkProfile;
        this.running = false;
        this._ready = false; // after "Hello in child"
        this._readyPromises = [];
        this._partialResponse = null;
        this._dataTimeout = null;
        this._onDataFunction = null;
        this._notReadyErrData = '';
        ...
        this._proc = (0, child_process_1.spawn)(sharkdCmd, spawnArgs, {
            cwd: sharkdDir,
                ...
                if (this._partialResponse) {
                    ...
                    if (crPos === 0) {
                        ...
                        try {
                            ...
                            console.log(`SharkdProcess(${this.id}) crPos = ${crPos} partialResponse.length=${this._partialResponse.length} got err=${err}` // partialResponse=${this._partialResponse?.toString()}`
                            );
                        }
                    }
                }
...
exports.SharkdProcess = SharkdProcess;
class WebsharkViewSerializer {
    constructor(reporter, treeViewProvider, _onDidChangeSelectedTime, sharkdPath, context, activeViews, callOnDispose) {
        this.reporter = reporter;
        this.treeViewProvider = treeViewProvider;
        this._onDidChangeSelectedTime = _onDidChangeSelectedTime;
        this.sharkdPath = sharkdPath;
...
exports.WebsharkViewSerializer = WebsharkViewSerializer;
class WebsharkViewCustomDocument {
    constructor(uri) {
        this.uri = uri;
...
class WebsharkViewReadonlyEditorProvider {
    constructor(reporter, treeViewProvider, _onDidChangeSelectedTime, context, activeViews, callOnDispose) {
        this.reporter = reporter;
        this.treeViewProvider = treeViewProvider;
        this._onDidChangeSelectedTime = _onDidChangeSelectedTime;
        this.context = context;
    ...
    resolveCustomEditor(document, webviewPanel, token) {
        ...
        const _wiresharkProfile = vscode.workspace.getConfiguration().get('vsc-webshark.wiresharkProfile');
...
exports.WebsharkViewReadonlyEditorProvider = WebsharkViewReadonlyEditorProvider;
;
;
class WebsharkView {
    get timeAdjustMs() { return this._timeAdjustMs; }
    ; // readonly
    constructor(panel, context, treeViewProvider, _onDidChangeSelectedTime, uri, _sharkd, activeViews, callOnDispose) {
        this.context = context;
        this.treeViewProvider = treeViewProvider;
        this._onDidChangeSelectedTime = _onDidChangeSelectedTime;
        this.uri = uri;
        this._sharkd = _sharkd;
        this.callOnDispose = callOnDispose;
        this._gotAliveFromPanel = false;
        this._msgsToPost = []; // msgs queued to be send to panel once alive
        this._pendingResponses = [];
        this._sharkd2Cbs = [];
        this._utcTimeColumnIdx = -1;
        this._firstInfosLoaded = false;
        this._timeAdjustMs = 0;
        // time sync support:
        this.gotTimeSyncEvents = false; // we reacted to time sync events ( so manually adjusting doesn't make sense as it will change back autom.)
        this._timeSyncEvents = [];
        this.lastReqId = 0;
        this._treeViewProvider = treeViewProvider;
        this._treeNode = new treeViewProvider_1.TreeViewNode(path.basename(uri.fsPath), null, 'file-binary');
        this._eventsNode = new treeViewProvider_1.TreeViewNode('events', this._treeNode, 'symbol-event');
        ...
        this.panel.webview.onDidReceiveMessage((e) => {
            ...
            switch (e.message) {
                ...
                case 'time update':
                    try {
                        // post time update...
                        const time = new Date(new Date(e.time).valueOf() + this._timeAdjustMs);
        ...
        if (htmlFile.length) {
            let htmlString = htmlFile.toString().replace(/{{localRoot}}/g, this.panel.webview.asWebviewUri(vscode.Uri.file(path.join(context.extensionPath, 'web'))).toString());
        ...
        const configColumns = (vscode.workspace.getConfiguration().get("vsc-webshark.columns"));
        const configColumnsWidths = (vscode.workspace.getConfiguration().get("vsc-webshark.columnsWidths"));
        ...
        this.sharkd2Request({ req: 'load', file: uri.fsPath }, (res) => {
            ...
            this.sharkd2Request({ req: 'info' }, (res) => {
                if ('error' in res) {
                    ...
                    if (this._utcTimeColumnIdx < 0) {
                        ...
                        this.sharkd2Request({ req: 'status' }, (res) => {
                            if ('error' in res) {
                                ...
                                this.sharkd2Request({ req: 'frames', limit: 1, column0: this._utcTimeColumnIdx }, (res) => {
                                    try {
                                        if ('error' in res) {
    ...
    dispose() {
        ...
        if (idx >= 0) {
            this._treeViewProvider.treeRootNodes.splice(idx, 1);
            this._treeViewProvider.updateNode(null);
        }
        this.callOnDispose(this);
    }
    onConfigColumnsChange(configColumns, configColumnsWidths) {
    ...
    updateTimeIndices(filter) {
        console.log(`WebsharkView.updateTimeIndices(filter='${filter}')`);
        let req = { req: 'intervals', interval: 1000 /* sec */ }; // todo if this ever gets too large we might have to add hours as interims.
        if (filter && filter.length > 0) {
            req.filter = filter;
        }
        this.sharkd2Request(req, (res) => {
            if ('error' in res) {
    ...
    getFrameIdxForTime(time) {
        ...
        return new Promise((resolve) => {
            const activeFilterStr = this._activeFilter ? this._activeFilter : '';
            const timeIntsFilterStr = this._timeIntsBySec && this._timeIntsBySec.filter ? this._timeIntsBySec.filter : '';
            if (!this._timeIntsBySec || (activeFilterStr !== timeIntsFilterStr)) {
                resolve(null);
                return;
            }
            if (!this._firstFrameTime || !this._fileStatus) {
                resolve(null);
                return;
            }
            ...
            if (firstFrameTimeVal > timeVal) {
                resolve(0);
                console.warn(`WebsharkView.getFrameIdxForTime returning 0 due to > time`);
                return;
            }
            if (firstFrameTimeVal + durationMs < timeVal) {
                resolve(null);
                console.warn(`WebsharkView.getFrameIdxForTime returning null due to last < time`);
                return;
            }
            // ok. we're within...
            ...
            // load the first frame to get the abs time reference:
            this.sharkd2Request({ req: 'frames', filter: this._activeFilter, skip: startIdx, limit: timeInts[i][1], column0: this._utcTimeColumnIdx }, (res) => {
                if ('error' in res) {
                    console.warn(`WebsharkView sharkd2 'frames' got error=${JSON.stringify(res)}`);
                }
                else {
    ...
    async handleDidChangeSelectedTime(ev) {
            ...
            if (ev.time.valueOf() > 0) {
                ...
                if (this._firstFrameTime !== undefined) {
                    this.getFrameIdxForTime(new Date(ev.time.valueOf())).then((idx) => {
                        ...
                        if (idx) {
                            this.postMsgOnceAlive({ command: "reveal frameIdx", frameIdx: idx });
                        }
                    });
                }
            }
    ...
    async onDidChangeConfiguration(ev) {
        const affected = ev.affectsConfiguration("vsc-webshark");
    ...
    async scanForEvents() {
        ...
        const events = vscode.workspace.getConfiguration().get("vsc-webshark.events");
        ...
        if (events && events.length) {
            ...
            for (let i = 0; i < events.length; ++i) {
                ...
                if (event.displayFilter?.length > 0) {
                    ...
                    this.sharkd2Request(req, (reqRes) => {
                        if ('error' in reqRes) {
                            ...
                            var convValuesFunction = (arr) => { return arr.join(' '); };
                            ...
                            for (let e = 0; e < res.length; ++e) {
                                try {
                                    ...
                                    if (event.level > 0) {
                                        // determine label:
                                        let label = event.label !== undefined ? stringFormat(event.label, frame.c.slice(1)) : frame.c[1];
                                        const node = new treeViewProvider_1.TreeViewNode(label, this._eventsNode);
                                    ...
                                    if (event.timeSyncId?.length > 0 && event.timeSyncPrio > 0) {
                                        // store as timeSync
                                        let timeSyncValue = frame.c.length > 2 ? (convValuesFunction(frame.c.slice(2)) /*frame.c.slice(2).join(' ')*/) : frame.c[1];
                                ...
                                const getParent = (level) => {
                                    if (level === 1) {
                                        ...
                                        if (parent.children.length === 0) {
                                            ...
                                            parent.children.push(new treeViewProvider_1.TreeViewNode(`(no parent level ${level - 1} event)`, parent));
                                        }
    ...
    async adjustTimeReq(e) {
        let curAdjustMs = this._timeAdjustMs;
        const time = new Date(new Date(e.time).valueOf() + curAdjustMs);
        ...
        if (!this.gotTimeSyncEvents && this.lastSelectedTimeEv) {
            if (time) {
                ...
                let response = await vscode.window.showInformationMessage(`Adjust based on last received time event (adjust by ${selTimeAdjustValue / 1000} secs)?`, { modal: true }, "yes", "no");
                if (response === "yes") {
        ...
        if (doManualPrompt) {
            vscode.window.showInputBox({ prompt: `Enter new time adjust in secs (cur = ${curAdjustMs / 1000}):`, value: (curAdjustMs / 1000).toString() }).then(async (value) => {
                if (value) {
                    let newAdjustMs = (+value) * 1000;
...
exports.WebsharkView = WebsharkView;
...
//# sourceMappingURL=websharkView.js.map